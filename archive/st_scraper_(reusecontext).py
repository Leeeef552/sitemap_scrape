...
class ST_Scraper:
    ...
    async def _fetch_page_content(
        ...
        while attempt <= max_retries:
            # only sleep *before* retry attempts
            if attempt:
            ...
            async with self.semaphore:
                ...
                try:
                    ...
                    if "429" in msg or "too many requests" in msg:
                        # 1) try to read server‑suggested back‑off
                        raw = None
                        if response:
        ...
        # exhausted retries
        raise RuntimeError(
            f"Aborted after {max_retries + 1} attempts (rate‑limited): {url}"
        )
